...
import { ServiceTypeSelector } from "./dialog-selector-service-type.js"; // Dialogs

const setDestination = Symbol();
const setServiceType = Symbol(); // Texts on done
...
class SetServiceDialog extends React.Component {
  ...
  updateOutputDisplay() {
    ...
    const destination = this.state.savedDestination; // update output display in the parent component
  ...
  render() {
    ...
    }) : null, this.state.currentDialog === setServiceType ? React.createElement(ServiceTypeSelector, {
      line: line,
      destination: destination,
      serviceType: serviceType,
